        ...
        <TableBody>
          {pooList?.map((row: IPoolData) => (
            <StyledTableRow onClick={_ => navigate(`/${chain}/Pools/${row.pool}`)} key={`${row.symbol0}-${row.symbol1}`}>
              ...
              <StyledTableCell>${row.tvl * (usdPrice ?? 0)}</StyledTableCell>
              <StyledTableCell>${row.volume24H * (usdPrice ?? 0)}</StyledTableCell>
              <StyledTableCell>${row.volume7D * (usdPrice ?? 0)}</StyledTableCell> 
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
